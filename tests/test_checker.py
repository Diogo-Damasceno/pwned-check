...
from pwned_check.checker import match_suffix_count, sha1_hex, split_hash
...
def test_match_suffix_count_found():
    response = "1E4C9B93F3F0682250B6CF8331B7EE68FD8:120\n" \
               "ABCDEF000000000000000000000000000000:3\n"
    suffix = sha1_hex("password")[5:]
    assert match_suffix_count(response, suffix) == 120


def test_match_suffix_count_not_found():
    response = "ABCDEF000000000000000000000000000000:3\n"
    suffix = sha1_hex("password")[5:]
    assert match_suffix_count(response, suffix) == 0


def test_match_suffix_count_case_insensitive():
    response = "1e4c9b93f3f0682250b6cf8331b7ee68fd8:42\n"
    suffix = sha1_hex("password")[5:]
    assert match_suffix_count(response, suffix) == 42
